...
pub async fn get_resorts(
    ...
    let resorts_result = sqlx::query!(
        ...
        SELECT
            ...
            CAST(NULL AS CHAR) AS region,
            CAST(NULL AS CHAR) AS continent,
            CAST(NULL AS DOUBLE) AS latitude,
            CAST(NULL AS DOUBLE) AS longitude,
            CAST(NULL AS SIGNED) AS village_altitude_m,
            CAST(NULL AS SIGNED) AS min_altitude_m,
            CAST(NULL AS SIGNED) AS max_altitude_m,
            CAST(NULL AS CHAR) AS ski_area_name,
            CAST(NULL AS CHAR) AS ski_area_type,
            CAST(NULL AS CHAR) AS official_website,
            CAST(NULL AS CHAR) AS lift_status_url,
            CAST(NULL AS CHAR) AS slope_status_url,
            CAST(NULL AS CHAR) AS snow_report_url,
            CAST(NULL AS CHAR) AS weather_url,
            CAST(NULL AS CHAR) AS status_provider,
            CAST(NULL AS CHAR) AS status_last_scraped_at,
            CAST(NULL AS SIGNED) AS lifts_open_count,
            CAST(NULL AS SIGNED) AS slopes_open_count,
            CAST(NULL AS SIGNED) AS snow_depth_valley_cm,
            CAST(NULL AS SIGNED) AS snow_depth_mountain_cm,
            CAST(NULL AS SIGNED) AS new_snow_24h_cm,
            CAST(NULL AS DOUBLE) AS temperature_valley_c,
            CAST(NULL AS DOUBLE) AS temperature_mountain_c
        FROM resorts
        ORDER BY name
        "#
    )
    ...
    let resorts = match resorts_result {
        Ok(rows) => rows
            ...
            .map(|row| Resort {
                ...
                village_altitude_m: row.village_altitude_m.map(|v| v as i32),
                min_altitude_m: row.min_altitude_m.map(|v| v as i32),
                max_altitude_m: row.max_altitude_m.map(|v| v as i32),
                ...
                lifts_open_count: row.lifts_open_count.map(|v| v as i32),
                slopes_open_count: row.slopes_open_count.map(|v| v as i32),
                snow_depth_valley_cm: row.snow_depth_valley_cm.map(|v| v as i16),
                snow_depth_mountain_cm: row.snow_depth_mountain_cm.map(|v| v as i16),
                new_snow_24h_cm: row.new_snow_24h_cm.map(|v| v as i16),
